...
def test_all_checks_pass_accepts():
    doc = DocumentMatchResult(overall_match=True, id_number_match=True, name_match=True, name_similarity=1.0)
...
def test_document_mismatch_rejects():
    doc = DocumentMatchResult(
        overall_match=False, id_number_match=False, name_match=True,
        name_similarity=1.0, reasons=["User-supplied ID number does not match the ID document."]
    )
    face = FaceMatchResult(success=True, is_match=True, confidence=0.95)
...
def test_both_checks_fail_reports_both_reasons():
    doc = DocumentMatchResult(
        overall_match=False, id_number_match=False, name_match=False,
        name_similarity=0.2, reasons=["Name similarity 0.20 is below the 0.85 threshold."]
    )
