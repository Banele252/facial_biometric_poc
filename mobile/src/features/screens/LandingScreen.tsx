...
const CAROUSEL_W = DESIGN_WIDTH - 24;
...
const slides = [
  {
    id: '1',
    image: 'https://images.pexels.com/photos/5053850/pexels-photo-5053850.jpeg?auto=compress&cs=tinysrgb&w=1400',
    badge: 'Biometric',
  ...
  {
    id: '2',
    image: 'https://images.pexels.com/photos/5926392/pexels-photo-5926392.jpeg?auto=compress&cs=tinysrgb&w=1400',
    badge: 'Risk rules',
  ...
  {
    id: '3',
    image: 'https://images.pexels.com/photos/7191988/pexels-photo-7191988.jpeg?auto=compress&cs=tinysrgb&w=1400',
    badge: 'Auditable',
  ...
  {
    id: '4',
    image: 'https://images.pexels.com/photos/5244048/pexels-photo-5244048.jpeg?auto=compress&cs=tinysrgb&w=1400',
    badge: 'Compliance',
  ...
  {
    id: '5',
    image: 'https://images.pexels.com/photos/7054505/pexels-photo-7054505.jpeg?auto=compress&cs=tinysrgb&w=1400',
    badge: 'Adaptive',
  ...
  {
    id: '6',
    image: 'https://images.pexels.com/photos/3184292/pexels-photo-3184292.jpeg?auto=compress&cs=tinysrgb&w=1400',
    badge: 'Reusable',
...
const actions = [
  ...
  { id: 'verify', glyph: 'person-outline', label: 'Verify ID', screen: 'VerifyDetailsScreen' },
  { id: 'esim', glyph: 'cellular-outline', label: 'eSIM', screen: 'ScanSimScreen' },
  { id: 'wallet', glyph: 'wallet-outline', label: 'Wallet', screen: 'ConsentScreen' },
];
...
const streamData = [
  {
    ...
    body: 'Passwordless, biometric and MFA sign-in',
    detail:
        'Sign in to MTN and partner services with your face or fingerprint instead of a password or OTP. Step-up MFA kicks in only for high-risk actions.',
    tags: ['Per authentication', 'Biometric'],
  },
  {
    id: 'onboard',
    glyph: 'create-outline',
    title: 'Digital Onboarding',
    body: 'SIM, fibre, insurance — verified in minutes',
    detail:
        'One verified identity opens every MTN product. RICA-compliant onboarding for SIM, fibre, insurance and lending without a store visit.',
    tags: ['Subscription / API fees', 'RICA ready'],
  },
  {
    id: 'enterprise',
    glyph: 'code-working-outline',
    title: 'Enterprise Identity APIs',
    body: 'KYC, liveness and device trust, pay-per-call',
    detail:
        'Banks, insurers and retailers call our APIs to verify a customer, check liveness or score device trust — billed per call, no integration lock-in.',
  ...
  },
  {
    ...
    body: 'Secure profile download and activation',
    detail:
        'Download an eSIM profile straight to your device after a biometric check — no plastic SIM, no courier, active in about two minutes.',
...
export default function LandingScreen({ navigate }: LandingScreenProps) {
  ...
  const allReady = layoutReady && imagesReady >= SLIDES;

  useEffect(() => {
    audit.init(sessionId).then(() => {
      audit.log('JOURNEY_STARTED', { metadata: { source: 'app_launch' } });
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all(slides.map((s) => Image.prefetch(s.image).catch(() => {}))).then(() => {
      if (!cancelled) setImagesReady(SLIDES);
    });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    ...
    autoPlayRef.current = setInterval(() => {
      ...
      const next = (currentIndexRef.current + 1) % SLIDES;
      flatListRef.current?.scrollToOffset({
        offset: next * CAROUSEL_W,
        animated: true,
      });
  ...
  const onScrollEnd = useCallback(
      (event: any) => {
        ...
        const index = Math.round(x / CAROUSEL_W);
        if (index >= 0 && index < SLIDES && index !== currentIndex) {
  ...
  // ─────────────────────────────────────────────────────────────
  // Swap SIM button – navigates directly (token logic removed)
  // ─────────────────────────────────────────────────────────────
  const handleCtaPress = useCallback(
      (e: any) => {
        ...
        setPhase('busy');
        audit.log('SCREEN_VIEWED', { action: 'cta_swap_sim_pressed' });

        // Direct navigation without token exchange
        setPhase('done');
  ...
  const handleImageLoad = useCallback(
      () => setImagesReady((p) => Math.min(p + 1, SLIDES)),
      [],
  );
  ...
  const renderSlide = ({ item }: { item: (typeof slides)[0] }) => (
      <View style={styles.slideContainer}>
        ...
        />
        <LinearGradient
            colors={['rgba(10,10,11,0)', 'rgba(10,10,11,0.78)', 'rgba(10,10,11,0.96)']}
            locations={[0, 0.52, 1]}
  ...
  const renderStream = (stream: (typeof streamData)[0]) => {
    ...
    return (
        <View
            key={stream.id}
            style={[styles.accordionItem, { borderTopWidth: stream.id === 'auth' ? 0 : 1 }]}
        >
          <Pressable style={styles.accordionHeader} onPress={() => toggleStream(stream.id)}>
            <View
                style={[
                  ...
                  {
                    backgroundColor: isOpen ? '#FFF3C9' : '#F4F4F7',
                    borderColor: isOpen ? '#F2DC8E' : '#EFEFF3',
                  },
                ]}
            >
  ...
  const ctaText =
      phase === 'busy' ? 'Securing…' : phase === 'done' ? "Let's go" : 'Swap SIM';
  ...
  return (
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        ...
        >
          ...
          <View style={styles.carouselWrapper} onLayout={() => setLayoutReady(true)}>
            {!allReady && (
          ...
          <View style={styles.ctaRow}>
            <TouchableOpacity
                style={[
                  styles.ctaButton,
                  phase === 'busy' && styles.ctaBusy,
                  phase === 'done' && styles.ctaDone,
                ]}
            ...
            >
              {ripples.map((r) => (
                  <Animated.View
                      key={r.id}
                      style={[styles.ripple, { left: r.x - 260, top: r.y - 260 }]}
                  />
              ))}
              <View style={styles.ctaIconWrap}>
                <Ionicons name="sync-outline" size={22} color="#FFCB05" />
              </View>
              <View style={styles.ctaTextWrap}>
                <Text style={styles.ctaLabel}>{ctaText}</Text>
                {phase === 'busy' && (
                    <ActivityIndicator size="small" color="#111114" style={styles.ctaSpinner} />
                )}
                {phase === 'done' && (
                    <View style={styles.ctaCheck}>
                      <Ionicons name="checkmark" size={14} color="#FFCB05" />
                    </View>
                )}
              </View>
            </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  actionGrid: {
    ...
    paddingHorizontal: 6,
    gap: 4,
    shadowColor: 'rgba(17,17,20,0.18)',
